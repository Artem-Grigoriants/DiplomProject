#Код файла `views.py` определяет представления API для приложения `orders`.
#Эти представления обрабатывают HTTP-запросы и предоставляют конечные точки(endpoint)
#для различных операций, связанных с заказами, корзинами, товарами и управлением пользователями.
...
from .models import Contact, Order, OrderItem, Cart, CartItem, Shop
...
class OrderListView(APIView):
    ...
    def post(self, request):
        ...
        order = Order.objects.create(user=request.user, status='new')
        for it in items:
            product = get_object_or_404(ProductBase, id=it.get('product'))
            shop = Shop.objects.first() or Shop.objects.create(name='Default Shop')
            OrderItem.objects.create(
                order=order,
                product=product,
                shop=shop,
